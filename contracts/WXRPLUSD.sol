...
abstract contract BasicToken is Ownable, ERC20Basic {
    ...
    function balanceOf(
        address _owner
    ) public virtual override returns (uint balance) {
...
abstract contract StandardToken is BasicToken, ERC20 {
    ...
    function allowance(
        address _owner,
        address _spender
    ) public virtual override returns (uint remaining) {
...
contract WXRPLUSD is Pausable, StandardToken, BlackList {
    using SafeMath for uint256;
    string public name;
    ...
    function transfer(
        ...
        if (deprecated) {
            // Assuming UpgradedStandardToken is another contract with a compatible transfer function
    ...
    function balanceOf(
        address who
    ) public override(BasicToken, ERC20Basic) returns (uint) {
        if (deprecated) {
    ...
    function allowance(
        address _owner,
        address _spender
    ) public override returns (uint remaining) {
        if (deprecated) {
